...
namespace Weather
{
    public partial class Form1 : Form
    {
        ...
        private void cboCity_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                using (XmlReader xr = XmlReader.Create(strURL))
                {
                            ...
                            case XmlNodeType.CDATA:
                                {
                                    ...
                                    if (xr.Value.Contains("비"))
                                    {
                                        richTextBox1.Text += " 비가 내릴 예정이니 우산을 챙겨주세요";
                                        label18.Text = "우산을 챙겨주세요!";
                                    }
                                    break;
                                }
                    ...
                    w1 = new Wt(strWh1[0], int.Parse(strWh1[2]), int.Parse(strWh1[3]), strWh1[1]);
                    w2 = new Wt(strWh2[0], int.Parse(strWh2[2]), int.Parse(strWh2[3]), strWh2[1]);
                    w3 = new Wt(strWh3[0], int.Parse(strWh3[2]), int.Parse(strWh3[3]), strWh3[1]);
                    w4 = new Wt(strWh4[0], int.Parse(strWh4[2]), int.Parse(strWh4[3]), strWh4[1]);
                    w5 = new Wt(strWh5[0], int.Parse(strWh4[2]), int.Parse(strWh4[3]), strWh4[1]);
                    w6 = new Wt(strWh6[0], int.Parse(strWh6[2]), int.Parse(strWh6[3]), strWh6[1]);
                    ...
                    label25.Text = w5.Date;
                    label26.Text = "최저: " + w5.MinTemp + "Cº";
                    label27.Text = "최고: " + w5.MaxTemp + "Cº";
                    label28.Text = w5.Dayday;

                    label29.Text = w6.Date;
                    label30.Text = "최저: " + w6.MinTemp + "Cº";
                    label31.Text = "최고: " + w6.MaxTemp + "Cº";
                    label32.Text = w6.Dayday;
            ...
            catch (Exception ex)
            {
                ...
                //DataManager.printlog(contents, Name);
            }
        }

        private void ClothesToday(Wt weat)
        {
            double mid = (weat.MinTemp + weat.MaxTemp)/2;
            if (28 <= mid)
            {
        ...
        //라벨의 태그 번호를 설정해서 라벨을 클릭하면 해당 케이스로 이동하도록 설정함
